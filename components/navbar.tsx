      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link href={user ? '/dashboard' : '/'} className="flex items-center gap-2 group">
            <div className="w-10 h-10 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/20 group-hover:scale-110 transition-transform duration-300">
          ...
          {user && (
            <div className="hidden md:flex items-center gap-6">
              ...
              <Link
                href="/generate"
                className={getDesktopNavClass('/generate')}
              >
                Generate
              </Link>
              <Link
                href="/documents"
                className={getDesktopNavClass('/documents')}
              >
                Documents
              </Link>
              <Link
                href="/pricing"
                className={getDesktopNavClass('/pricing')}
              >
                Pricing
              </Link>
              <Link
                href="/support"
                className={getDesktopNavClass('/support')}
              >
                Support
              </Link>
              {user.role === 'ADMIN' && (
                <>
                  <Link
                    href="/admin"
                    className={getDesktopNavClass('/admin')}
                  >
                    Admin
                  </Link>
              ...
              <>
                ...
                <div className="hidden md:flex items-center gap-4">
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <div className={`mt-1 flex items-center gap-1.5 px-2.5 py-1 rounded-lg border transition-all duration-300 ${
                      user.subscriptionTier === 'ENTERPRISE' ? 'bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800 text-purple-600 dark:text-purple-400' :
                      user.subscriptionTier === 'PRO' ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800 text-amber-600 dark:text-amber-400' :
                      user.subscriptionTier === 'STARTER' ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400' :
                ...
                <button
                  ...
                  className="md:hidden p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-all active:scale-95 border border-transparent hover:border-border/50"
        ...
        {/* Mobile Menu */}
        {user && mobileMenuOpen && (
          <div className="md:hidden py-6 border-t border-gray-100 dark:border-gray-800 animate-in slide-in-from-top duration-300">
