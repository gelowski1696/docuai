...
export function StatsSection({ documentCount, totalTokens, activeTemplatesCount }: StatsSectionProps) {
  ...
  return (
    <section className="glass rounded-[3rem] p-12 border border-border/50 relative overflow-hidden group">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6 relative">
        <div>
          <h3 className="text-3xl font-extrabold mb-2">Workspace Analytics</h3>
          <p className="text-gray-500 dark:text-gray-400 text-lg">Real-time insight into your production pipeline.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 text-center relative">
        <div className="p-8 rounded-[2rem] bg-background/40 border border-border/50 hover:bg-background/60 transition-colors">
          <div className="text-sm font-bold text-gray-400 uppercase tracking-[0.2em] mb-4">Documents</div>
          <div className="text-5xl font-black gradient-text">{documentCount}</div>
        </div>
        <div className="p-8 rounded-[2rem] bg-background/40 border border-border/50 hover:bg-background/60 transition-colors">
          <div className="text-sm font-bold text-gray-400 uppercase tracking-[0.2em] mb-4">Total Tokens</div>
          <div className="text-5xl font-black gradient-text">{totalTokens.toLocaleString()}</div>
        </div>
        <div className="p-8 rounded-[2rem] bg-background/40 border border-border/50 hover:bg-background/60 transition-colors">
          <div className="text-sm font-bold text-gray-400 uppercase tracking-[0.2em] mb-4">Active Templates</div>
          <div className="text-5xl font-black gradient-text">{activeTemplatesCount}</div>
        </div>
      </div>
    </section>
  );
